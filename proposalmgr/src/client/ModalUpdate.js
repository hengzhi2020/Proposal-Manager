...
class ModalUpdate extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      cycle:
        !!this.props.updateProposal[0].cycle === false
          ? 1
          : this.props.updateProposal[0].cycle,
  ...
  updatedToServer = () => {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}proposals`, {
      ...
      if (!response.ok) {
        throw new Error(
          'HTTP error - to proposal table, status = ' + response.status
        );
      }
            ...
            <Form className="modal-container">
              ...
              <Form.Group unstackable widths={2}>
                <Form.Input
                  ...
                  name="VA_sponsor"
                  value={
                    this.state.VA_sponsor === null
                      ? 'sponsor'
                      : this.state.VA_sponsor
                  }
                  onChange={(e) =>
                    this.setState({ VA_sponsor: e.target.value })
                  }
                />
                <Form.Input
                  label="Support"
                  ...
                  name="support"
                  value={
                    this.state.support === null ? 'support' : this.state.support
                  }
              ...
              <Form.Group unstackable widths={2}>
                <Form.Input
                  ...
                  value={
                    this.state.project_presenter === null
                      ? 'presenter'
                      : this.state.project_presenter
                  }
                  onChange={(e) =>
                    this.setState({ project_presenter: e.target.value })
                  }
                />
                <Form.Input
