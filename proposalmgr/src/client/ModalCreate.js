...
class ModalCreate extends React.Component {
  ...
  createdToServer = () => {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}reviewdata`, {
      ...
      if (!response.ok) {
        throw new Error(
          'HTTP error - to reviewdata table, status = ' + response.status
        );
      }
      console.log(response);
    });
    ...
    return (
      <div>
        <Modal id="modal_create" open={this.props.createClickopen}>
          <Modal.Header>Create a Proposal: by C-DACS admin only</Modal.Header>
