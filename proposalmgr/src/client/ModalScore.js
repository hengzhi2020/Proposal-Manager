...
class ModalScore extends React.Component {
  ...
  saveReviewToServer = () => {
    // console.log('this.state.submitFlag-SAVE', this.state.submitFlag);
    if (this.props.DATA.submitFlag === 'true') {
      alert(
        'You have submitted your review. Update is not allowed. Thank you!'
      );
    }
    ...
    if (this.state.submitFlag === 'false') {
      ...
      alert(
        'Save your review (Scores _&_ Comments), and you can update it later. Thank you!'
      );
    }
  };

  submitReviewToServer = () => {
    // console.log('this.state.submitFlag-SUBMIT', this.state.submitFlag);
    if (this.props.DATA.submitFlag === 'true') {
      alert(
        'You have submitted your review. Update is not allowed. Thank you!'
      );
      ...
      (!!this.state.score1 || this.state.score1 === 0) &&
      (!!this.state.score2 || this.state.score2 === 0) &&
      (!!this.state.score3 || this.state.score3 === 0) &&
      (!!this.state.score4 || this.state.score4 === 0) &&
      (!!this.state.score5 || this.state.score5 === 0)
    ) {
      const reviewdata = {
        proposal_id: this.props.scoreProposal[0].id,
        reviewer_id: this.props.DATA.Reviewer.id,
      ...
      window.location.reload(true);
      alert(
        'You have successfully submitted your review (Scores and Comments). Thank you!'
      );
    } else {
      alert(
        'Please give a score to each of the 5 aspects for SUBMIT. Thank you!'
      );
    }
  };
      ...
      panes = [
        {
          menuItem: (
            <Menu.Item key="final-report">
              <p className="menuitems" id="menuitem7">
                - LIVE Summary -
              </p>
            </Menu.Item>
          ),
            ...
            content: (
              <div>
                <CommAdd7
                  ...
                  proposalPresenter={
                    this.props.scoreProposal[0].project_presenter
                  }
        ...
        {
          menuItem: (
            <Menu.Item key="message-1">
              <p className="menuitems">1. Business Need </p>
            </Menu.Item>
          ),
          pane: {
            key: 'tab1',
            content: (
              <div>
                ...
                />
                <CommAdd1 getComm1={this.getComm1} comm1={this.state.comm1} />
              </div>
            ),
          },
        },
        {
          menuItem: (
            <Menu.Item key="message-2">
              <p className="menuitems">2. Feasibility & Goals</p>
            </Menu.Item>
          ),
          pane: {
            key: 'tab2',
            content: (
              <div>
                ...
                />
                <CommAdd2 getComm2={this.getComm2} comm2={this.state.comm2} />
              </div>
            ),
          },
        },
        {
          menuItem: (
            <Menu.Item key="message-3">
              <p className="menuitems">3. Resources</p>
            </Menu.Item>
          ),
          pane: {
            key: 'tab3',
            content: (
              <div>
                ...
                />
                <CommAdd3 getComm3={this.getComm3} comm3={this.state.comm3} />
              </div>
            ),
          },
        },
        {
          menuItem: (
            <Menu.Item key="message-4">
              <p className="menuitems">4. Commitment</p>
            </Menu.Item>
          ),
          pane: {
            key: 'tab4',
            content: (
              <div>
                ...
                />
                <CommAdd4 getComm4={this.getComm4} comm4={this.state.comm4} />
              </div>
            ),
          },
        },
        {
          menuItem: (
            <Menu.Item key="message-5">
              <p id="menuitem5">5. Constrains</p>
            </Menu.Item>
          ),
          pane: {
            key: 'tab5',
            content: (
              <div>
                ...
                />
                <CommAdd5 getComm5={this.getComm5} comm5={this.state.comm5} />
              </div>
            ),
          },
        },
        {
          menuItem: (
            <Menu.Item key="message-6" onClick={this.getScore6}>
              <p className="menuitems">6. Overall Comments</p>
            </Menu.Item>
          ),
          pane: {
            key: 'tab6',
    ...
    return (
      <div>
        <Modal
          ...
          id="modal_score"
          size="fullscreen"
          open={this.props.scoreClickopen}
        >
          <Modal.Header>
            Reviewers: __ Please Give Scores and Add Comments to this Proposal
            __
          </Modal.Header>
          <Form className="modal_ID_Title">
            <Form.Group unstackable widths={2}>
              <Form.Input
                ...
                value={this.props.scoreProposal[0].id}
              />
              <Form.Input
                label="Title"
                placeholder="Title"
                name="title"
                value={this.props.scoreProposal[0].title}
              />
            </Form.Group>
          </Form>
          <div>
          ...
          <div className="score_comm_btns">
            <Button
              ...
              id="btn-save-scoring"
              className="ui button"
              onClick={this.saveReviewToServer}
            >
              SAVE _ your review and update it later
            </Button>
            <Button
              type="button"
              id="btn-submit-scoring"
              className="ui button"
              onClick={this.submitReviewToServer}
            >
              SUBMIT _ your scores and comments
            </Button>
            <button
              type="button"
              id="btn-close-scoring"
              className="ui button"
              onClick={this.props.scoreClickclose}
            >
              CLOSE _ this window
            </button>
          </div>
  ...
  updateDATAbyReviewerData = () => {
    let Selectedreviewdata = this.props.reviewdata.filter(
      (item) => +item.reviewer_id === +this.props.DATA.Reviewer.id
    );
    ...
    if (Selectedreviewdata.length === 0) {
      ...
      this.props.DATA.BusinessNeed.comments =
        Selectedreviewdata[0].business_comms;
      this.props.DATA.Feasibility.score =
        Selectedreviewdata[0].feasibility_score;
      this.props.DATA.Feasibility.comments =
        Selectedreviewdata[0].feasibility_comms;
      this.props.DATA.Resources.score = Selectedreviewdata[0].resources_score;
      this.props.DATA.Resources.comments =
        Selectedreviewdata[0].resources_comms;
      this.props.DATA.Commitment.score = Selectedreviewdata[0].commitment_score;
      this.props.DATA.Commitment.comments =
        Selectedreviewdata[0].commitment_comms;
      this.props.DATA.Constraints.score =
        Selectedreviewdata[0].constraints_score;
      this.props.DATA.Constraints.comments =
        Selectedreviewdata[0].constraints_comms;
      this.props.DATA.OverAllComments.typein =
        Selectedreviewdata[0].overall_comms;
      this.props.DATA.submitFlag = Selectedreviewdata[0].submitted;
      ...
      this.setState({
        ...
        comm5: this.props.DATA.Constraints.comments,
        comm6: this.props.DATA.OverAllComments.typein,
        submitFlag: this.props.DATA.submitFlag,
      });
      // console.log('this.props.DATA.submitFlag', this.props.DATA.submitFlag);
    }
  };
}
